...
class Lexer:
    def __init__(self, text):
        ...
        self.token_patterns = {
            ...
            'ID': r'[A-Za-z]([A-Za-z]|(_)*|[0-9])*',  # Corregido el patrón para ID
            'NUMBER': r'[0-9]+(\.[0-9]+)?(E[+\-]?[0-9]+)?',  # Corregido el patrón para NUMBER
            'SEMICOLON': r';',
            'ASSIGNOP': r':=',  # Corregido el patrón para ASSIGNOP
            'LT': r'<',
            'EQ': r'=',
            'PLUS': r'\+',  # Corregido el patrón para PLUS
            'MINUS': r'\-',  # Corregido el patrón para MINUS
            'TIMES': r'\*',  # Corregido el patrón para TIMES
            'DIV': r'/',  # Corregido el patrón para DIV
